...
class LLMLogger(BaseCallbackHandler):
    """Callback que registra início, término e erros das chamadas ao LLM."""
    ...
    def on_llm_start(self, serialized: dict, prompts: list[str], **kwargs: Any) -> None:
        ...
        logger.info("  ┌─ Início da chamada ao LLM | modelo=%s | mensagens=%d", model, len(prompts))
    ...
    def on_llm_end(self, response: LLMResult, **kwargs: Any) -> None:
        ...
        logger.info(
            "  └─ Chamada ao LLM concluída | %.1fs | motivo=%s | chars=%d | tokens: prompt=%s completion=%s total=%s",
            elapsed,
            finish,
            chars,
    ...
    def on_llm_error(self, error: BaseException, **kwargs: Any) -> None:
        logger.error("  └─ Erro na chamada ao LLM | %s: %s", type(error).__name__, error)
...
def get_llm_client(
    ...
    """Retorna (ChatOpenAI, nome_do_modelo).

    override_url/model têm precedência sobre os valores do .env,
    permitindo que o frontend informe endpoints locais customizados em tempo real.
    """
